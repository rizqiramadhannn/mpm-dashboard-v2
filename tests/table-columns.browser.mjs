...
test('column picker persistence, isolation, validation, sorting, dialogs and export', async () => {
  ...
  const server=http.createServer(async(req,res)=>{const isPdf=req.url==='/invoice/download/inv1';res.setHeader('Content-Type',isPdf?'application/pdf':req.url==='/bundle.js'?'text/javascript':'text/html');res.end(isPdf?'%PDF-1.4 test invoice':req.url==='/bundle.js'?await fs.readFile(path.join(directory,'bundle.js')):`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${css}:root{--font-geist-sans:Arial;--font-geist-mono:monospace}body{font-family:Arial}</style></head><body><div id="root"></div><script src="/bundle.js"></script></body></html>`)});
  ...
  try {
    ...
    await page.getByRole('button',{name:'Download Semua Invoice',exact:true}).click();
    const invoiceDialog=page.getByRole('dialog');assert.match(await invoiceDialog.textContent(),/1 SPH belum memiliki TTD Materai/);assert.match(await invoiceDialog.textContent(),/SPH001 — Customer/);const zipDownload=page.waitForEvent('download');await invoiceDialog.getByRole('button',{name:'Download ZIP',exact:true}).click();const zipFile=await zipDownload;const zipBytes=await fs.readFile(await zipFile.path());assert.equal(zipBytes.subarray(0,4).toString('hex'),'504b0304');
