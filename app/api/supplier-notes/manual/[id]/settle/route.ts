...
import { settleManualNote } from "../../../../../supplier/nota-manual/payment-storage";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await settleManualNote(await getDb(), id, payload, request.headers.get("cf-connecting-ip") ?? request.headers.get("x-forwarded-for")?.split(",")[0].trim() ?? "unknown");
    ...
    console.error("Manual note settlement failed");
    return NextResponse.json({ error: "Gagal menyimpan pelunasan; rekonsiliasi sebelum retry." }, { status: 500 });
  }
}
