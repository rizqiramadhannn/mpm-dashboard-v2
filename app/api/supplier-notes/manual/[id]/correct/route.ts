...
import { correctManualNote } from "../../../../../supplier/nota-manual/correction-storage";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await correctManualNote(await getDb(), id, payload, request.headers.get("cf-connecting-ip") ?? request.headers.get("x-forwarded-for")?.split(",")[0].trim() ?? "unknown");
    ...
    console.error("Manual note correction failed");
    return NextResponse.json({ error: "Gagal mengoreksi nota; rekonsiliasi sebelum retry." }, { status: 500 });
  }
}
