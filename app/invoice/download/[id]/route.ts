...
function wrapText(
  ...
  const words = options.slashBreaks
    ? partNumberTokens(display)
...
function multilineText(
  ...
  return lines
    .map((lineValue, index) => text(x, y - index * lineHeight, lineValue, size, font))
    .join("");
}
...
function buildContent(document: InvoiceDocument, items: InvoiceItem[]) {
  ...
  for (let index = 0; index < visibleRows; index += 1) {
    ...
    if (item) {
      ...
      const partNameLines = wrapText(item.partName, colD - colC - 12, 10, {
        maxLines: 3,
      });
